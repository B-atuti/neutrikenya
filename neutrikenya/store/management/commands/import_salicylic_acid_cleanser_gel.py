...
class Command(BaseCommand):
    help = 'Import Salicylic Acid Cleanser Gel images from static folder to media folder and create product'

    def handle(self, *args, **options):
        # Get the project root directory
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        
        # Source directory for cleanser gel images
        source_dir = os.path.join(project_root, 'static', 'images', 'SALICYLIC ACID CLEANSER GEL')
        
        # Create media directory for products if it doesn't exist
        media_dir = os.path.join(project_root, 'media', 'products')
        os.makedirs(media_dir, exist_ok=True)
        
        # Get Salicylic Acid category
        try:
            salicylic_acid_category = Category.objects.get(name='Salicylic Acid')
            self.stdout.write(self.style.SUCCESS(f"Found Salicylic Acid category: {salicylic_acid_category.name}"))
        except Category.DoesNotExist:
            self.stdout.write(self.style.ERROR("Salicylic Acid category not found. Please run create_salicylic_acid_category command first."))
            return
        
        # Create or get the product
        product_name = 'Salicylic Acid Cleanser Gel'
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                'slug': product_slug,
                'category': salicylic_acid_category,
                'price': 1400.00,
                'description': 'A gentle yet effective gel cleanser formulated with Salicylic Acid to deeply cleanse and exfoliate the skin. This refreshing formula helps to unclog pores, remove excess oil, and prevent breakouts while maintaining the skin\'s natural moisture balance. Perfect for daily use to keep your skin clean, clear, and healthy.',
                'product_line': 'Salicylic Acid',
                'skin_concern': 'Acne, Oily Skin, Clogged Pores',
                'is_available': True
            }
        )
