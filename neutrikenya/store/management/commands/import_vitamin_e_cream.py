...
class Command(BaseCommand):
    help = 'Import Vitamin E Cream images from static folder to media folder and create product'
    ...
    def handle(self, *args, **options):
        ...
        source_dir = os.path.join(project_root, 'static', 'images', 'Vitamin E CREAM')
        
        # Create media directory for products if it doesn't exist
        media_dir = os.path.join(project_root, 'media', 'products')
        os.makedirs(media_dir, exist_ok=True)
        
        # Get Vitamin E category
        try:
            vitamin_e_category = Category.objects.get(name='Vitamin E')
            self.stdout.write(self.style.SUCCESS(f"Found Vitamin E category: {vitamin_e_category.name}"))
        except Category.DoesNotExist:
            self.stdout.write(self.style.ERROR("Vitamin E category not found. Please run create_vitamin_e_category command first."))
            return
        
        # Create or get the product
        product_name = 'Vitamin E Cream'
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                'slug': product_slug,
                'category': vitamin_e_category,
                'price': 2000.00,
                'description': 'A nourishing cream enriched with Vitamin E to help protect and moisturize your skin. This rich formula helps to combat free radical damage, reduce the appearance of fine lines and wrinkles, and promote skin healing. The cream provides deep hydration while helping to improve skin elasticity and texture. Perfect for daily use to maintain healthy, glowing skin.',
                'product_line': 'Vitamin E',
