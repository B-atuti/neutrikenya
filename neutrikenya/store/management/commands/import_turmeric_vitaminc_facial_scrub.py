...
class Command(BaseCommand):
    help = 'Import Turmeric & Vitamin C Facial Scrub images from static folder to media folder and create product'

    def handle(self, *args, **options):
        # Get the project root directory
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        
        # Source directory for facial scrub images
        source_dir = os.path.join(project_root, 'static', 'images', 'Turmeric & Vitamin C Facial Scrub')
        
        # Create media directory for products if it doesn't exist
        media_dir = os.path.join(project_root, 'media', 'products')
        os.makedirs(media_dir, exist_ok=True)
        
        # Get or create facial care category
        try:
            facial_care_category = Category.objects.get(name='Facial Care')
            self.stdout.write(self.style.SUCCESS(f"Found existing facial care category: {facial_care_category.name}"))
        except Category.DoesNotExist:
            # Create new category with a unique slug
            base_slug = slugify('Facial Care')
            slug = base_slug
            counter = 1
            ...
            facial_care_category = Category.objects.create(
                name='Facial Care',
                slug=slug,
                description='Our range of natural facial care products for healthy, glowing skin'
            )
            self.stdout.write(self.style.SUCCESS(f"Created new facial care category: {facial_care_category.name}"))
        
        # Create or get the product
        product_name = 'Turmeric & Vitamin C Facial Scrub'
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                ...
                'price': 2500.00,
                'description': 'A gentle yet effective facial scrub that combines the brightening benefits of Vitamin C with the anti-inflammatory properties of Turmeric for smooth, glowing skin.',
