...
class Command(BaseCommand):
    help = 'Import Pro Retinol Eye Cream images from static folder to media folder and create product'

    def handle(self, *args, **options):
        # Get the project root directory
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        
        # Source directory for eye cream images
        source_dir = os.path.join(project_root, 'static', 'images', 'PRO RETINOL EYE CREAM')
        ...
        product_name = 'Pro Retinol Eye Cream'
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                ...
                'price': 2000.00,
                'description': 'A specialized eye cream formulated with retinol to target the delicate eye area. This gentle yet effective formula helps to reduce the appearance of fine lines, wrinkles, and dark circles around the eyes. The cream provides intense hydration while delivering the anti-aging benefits of retinol in a formulation specifically designed for the sensitive eye area.',
                'product_line': 'Pro Retinol',
                'skin_concern': 'Eye Area, Fine Lines, Dark Circles',
                'is_available': True
            }
        )
