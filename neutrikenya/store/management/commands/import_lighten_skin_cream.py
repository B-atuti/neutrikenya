...
class Command(BaseCommand):
    help = 'Import Lighten Skin Cream images from static folder to media folder and create product'
    ...
    def handle(self, *args, **options):
        ...
        source_dir = os.path.join(project_root, 'static', 'images', 'LIGHTEN SKIN CREAM')
        
        # Create media directory for products if it doesn't exist
        media_dir = os.path.join(project_root, 'media', 'products')
        os.makedirs(media_dir, exist_ok=True)
        
        # Get or create lighten skin category
        try:
            lighten_skin_category = Category.objects.get(name='Lighten Skin')
            self.stdout.write(self.style.SUCCESS(f"Found existing lighten skin category: {lighten_skin_category.name}"))
        except Category.DoesNotExist:
            self.stdout.write(self.style.ERROR("Lighten Skin category not found. Please run create_lighten_skin_category command first."))
            return
        
        # Create or get the product
        product_name = 'Lighten Skin Cream'
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                ...
                'price': 3000.00,
                'description': 'A rich, nourishing cream formulated with natural ingredients to help brighten and even out skin tone. This deeply moisturizing formula helps reduce the appearance of dark spots and hyperpigmentation while providing long-lasting hydration for a more radiant, glowing complexion.',
