...
class Command(BaseCommand):
    help = 'Import 24K Gold Eye Patch images from static folder to media folder and create product'

    def handle(self, *args, **options):
        # Get the project root directory
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        
        # Source directory for eye patch images
        source_dir = os.path.join(project_root, 'static', 'images', '24K GOLD EYE PATCH')
        
        # Create media directory for products if it doesn't exist
        media_dir = os.path.join(project_root, 'media', 'products')
        os.makedirs(media_dir, exist_ok=True)
        
        # Get Gold Series category
        try:
            gold_series_category = Category.objects.get(name='Gold Series')
            self.stdout.write(self.style.SUCCESS(f"Found Gold Series category: {gold_series_category.name}"))
        except Category.DoesNotExist:
            self.stdout.write(self.style.ERROR("Gold Series category not found. Please run create_gold_series_category command first."))
            return
        
        # Create or get the product
        product_name = '24K Gold Eye Patch'
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                'slug': product_slug,
                'category': gold_series_category,
                'price': 1700.00,
                'description': 'A luxurious set of 5 eye patches infused with 24K gold particles to target the delicate eye area. These premium patches help to reduce the appearance of fine lines, dark circles, and puffiness while providing intense hydration and brightening effects. Each patch is enriched with nourishing ingredients to leave your eye area looking refreshed, revitalized, and radiant.',
                'product_line': 'Gold Series',
