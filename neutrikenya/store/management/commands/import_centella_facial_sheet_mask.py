...
class Command(BaseCommand):
    help = 'Import Centella Facial Sheet Mask images from static folder to media folder and create product'

    def handle(self, *args, **options):
        # Get the project root directory
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        
        # Source directory for mask images
        source_dir = os.path.join(project_root, 'static', 'images', 'Centella Facial Sheet Mask')
        
        # Create media directory for products if it doesn't exist
        media_dir = os.path.join(project_root, 'media', 'products')
        os.makedirs(media_dir, exist_ok=True)
        
        # Get Facial Care category
        try:
            facial_care_category = Category.objects.get(name='Facial Care')
            self.stdout.write(self.style.SUCCESS(f"Found Facial Care category: {facial_care_category.name}"))
        except Category.DoesNotExist:
            self.stdout.write(self.style.ERROR("Facial Care category not found. Please run create_facial_care_category command first."))
            return
        
        # Create or get the product
        product_name = 'Centella Facial Sheet Mask (4pcs)'
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                'slug': product_slug,
                'category': facial_care_category,
                'price': 2400.00,
                'description': 'A soothing sheet mask infused with Centella Asiatica extract to help calm and repair the skin. Each pack contains 4 masks, perfect for weekly treatments. The mask helps to reduce redness, soothe irritation, and promote skin healing. The Centella Asiatica extract is known for its powerful anti-inflammatory and healing properties, making this mask ideal for sensitive or irritated skin.',
                'product_line': 'Facial Care',
                'skin_concern': 'Sensitive Skin, Redness, Irritation',
                'is_available': True
            }
        )
