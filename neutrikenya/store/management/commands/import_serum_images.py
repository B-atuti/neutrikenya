...
class Command(BaseCommand):
    help = 'Import serum images from static folder to media folder and create product'
    ...
    def handle(self, *args, **options):
        ...
        source_dir = os.path.join(project_root, 'static', 'images', 'Serum')
        
        # Create media directory for products if it doesn't exist
        media_dir = os.path.join(project_root, 'media', 'products')
        os.makedirs(media_dir, exist_ok=True)
        
        # Get or create serum category
        try:
            serum_category = Category.objects.get(name='Serums')
            self.stdout.write(self.style.SUCCESS(f"Found existing serum category: {serum_category.name}"))
        except Category.DoesNotExist:
            # Create new category with a unique slug
            base_slug = slugify('Serums')
            slug = base_slug
            counter = 1
            ...
            serum_category = Category.objects.create(
                name='Serums',
                slug=slug,
                description='Our range of concentrated serums for targeted skincare solutions'
            )
            self.stdout.write(self.style.SUCCESS(f"Created new serum category: {serum_category.name}"))
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                ...
                'price': 3500.00,
                'description': 'A potent Vitamin C serum that brightens skin tone, reduces dark spots, and provides antioxidant protection.',
