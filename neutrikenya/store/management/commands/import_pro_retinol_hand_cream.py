...
class Command(BaseCommand):
    help = 'Import Pro Retinol Hand Cream images from static folder to media folder and create product'

    def handle(self, *args, **options):
        # Get the project root directory
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        
        # Source directory for hand cream images
        source_dir = os.path.join(project_root, 'static', 'images', 'PRO RETINOL HAND CREAM')
        
        # Create media directory for products if it doesn't exist
        media_dir = os.path.join(project_root, 'media', 'products')
        os.makedirs(media_dir, exist_ok=True)
        
        # Get Pro Retinol category
        try:
            pro_retinol_category = Category.objects.get(name='Pro Retinol')
            self.stdout.write(self.style.SUCCESS(f"Found Pro Retinol category: {pro_retinol_category.name}"))
        except Category.DoesNotExist:
            self.stdout.write(self.style.ERROR("Pro Retinol category not found. Please run create_pro_retinol_category command first."))
            return
        
        # Create or get the product
        product_name = 'Pro Retinol Hand Cream'
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                'slug': product_slug,
                'category': pro_retinol_category,
                'price': 1400.00,
                'description': 'A luxurious hand cream enriched with retinol to help reduce the appearance of age spots, fine lines, and wrinkles on the hands. This deeply moisturizing formula helps to improve skin texture and elasticity while providing long-lasting hydration. The cream absorbs quickly without leaving a greasy residue, making it perfect for daily use.',
                'product_line': 'Pro Retinol',
                'skin_concern': 'Aging Hands, Dry Skin, Age Spots',
                'is_available': True
            }
        )
