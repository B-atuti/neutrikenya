...
class Command(BaseCommand):
    help = 'Import shampoo images from static folder to media folder and create product'

    def handle(self, *args, **options):
        # Get the project root directory
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        
        # Source directory for shampoo images
        source_dir = os.path.join(project_root, 'static', 'images', 'Shampoo')
        
        # Create media directory for products if it doesn't exist
        media_dir = os.path.join(project_root, 'media', 'products')
        os.makedirs(media_dir, exist_ok=True)
        
        # Get or create hair category
        try:
            hair_category = Category.objects.get(name='Hair')
            self.stdout.write(self.style.SUCCESS(f"Found existing hair category: {hair_category.name}"))
        except Category.DoesNotExist:
            # Create new category with a unique slug
            base_slug = slugify('Hair')
            slug = base_slug
            counter = 1
            ...
            hair_category = Category.objects.create(
                name='Hair',
                slug=slug,
                description='Our range of natural hair care products for healthy, beautiful hair'
            )
            self.stdout.write(self.style.SUCCESS(f"Created new hair category: {hair_category.name}"))
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                ...
                'price': 2500.00,
                'description': 'A gentle, natural shampoo that cleanses and nourishes your hair while maintaining its natural moisture balance.',
                'product_line': 'Hair Care',
                'skin_concern': 'Dry Hair, Dandruff',
                'is_available': True
            }
        )
