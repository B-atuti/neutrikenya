...
class Command(BaseCommand):
    help = 'Import Vitamin C Facial Mask images from static folder to media folder and create product'

    def handle(self, *args, **options):
        # Get the project root directory
        project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))))
        
        # Source directory for facial mask images
        source_dir = os.path.join(project_root, 'static', 'images', 'FACIAL MASK')
        ...
        product_name = 'Vitamin C Facial Mask'
        ...
        product, created = Product.objects.get_or_create(
            ...
            defaults={
                ...
                'price': 2300.00,
                'description': 'A revitalizing facial mask enriched with Vitamin C to brighten skin, reduce dark spots, and promote a radiant complexion. This powerful formula helps to even out skin tone, minimize the appearance of fine lines, and leave your skin looking refreshed and glowing.',
                'product_line': 'Vitamin C',
                'skin_concern': 'Dull Skin, Dark Spots, Uneven Skin Tone',
                'is_available': True
            }
        )
